...
QEMU_AUDIO = -device intel-hda -device hda-output
QEMU_GPU =
QEMU_FLAGS = -boot d -cdrom $(ISO_IMAGE) -m 256 -no-reboot -no-shutdown -vga virtio -display $(QEMU_DISPLAY) $(QEMU_NET) $(QEMU_USB) $(QEMU_AUDIO) $(QEMU_GPU)
DISK_IMG  ?= disk.img
...
WIFI_OBJ       = $(BUILD_DIR)/wifi.o
HDA_OBJ        = $(BUILD_DIR)/hda.o
...
APP_WIFICFG_OBJ  = $(BUILD_DIR)/gui_app_wificfg.o
...
APP_BROWSER_OBJ    = $(BUILD_DIR)/gui_app_browser.o
APP_UPDATER_OBJ    = $(BUILD_DIR)/gui_app_updater.o
...
THEME_OBJ          = $(BUILD_DIR)/gui_theme.o
...
compile: $(FONT_HEADER) $(ICON_HEADERS) $(BUILD_DIR) ## compila y enlaza el kernel en build/kernel.bin
	...
	$(CC) $(CFLAGS) -c drivers/net/wifi.c -o $(WIFI_OBJ)
	$(CC) $(CFLAGS) -c drivers/audio/hda.c -o $(HDA_OBJ)
	$(CC) $(CFLAGS) -c drivers/audio/audio.c -o $(AUDIO_OBJ)
	...
	$(CC) $(CFLAGS) -c gui/apps/wificfg.c -o $(APP_WIFICFG_OBJ)
	...
	$(CC) $(CFLAGS) -c gui/apps/browser.c -o $(APP_BROWSER_OBJ)
	$(CC) $(CFLAGS) -c gui/apps/updater.c -o $(APP_UPDATER_OBJ)
	...
	$(CC) $(CFLAGS) -c gui/theme.c -o $(THEME_OBJ)
